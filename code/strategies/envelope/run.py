...
from utilities.okx_futures import OKXFutures
...
tracker_file = f"ChumbaTrading/code/strategies/envelope/tracker_{params['symbol'].replace('/', '-').replace(':', '-')}.json"
...
# --- CANCEL OPEN ORDERS ---
orders = okx.fetch_open_orders(params['symbol'])
for order in orders:
    okx.cancel_order(order['id'], params['symbol'])
trigger_orders = okx.fetch_open_trigger_orders(params['symbol'])
long_orders_left = 0
short_orders_left = 0
for order in trigger_orders:
    if order['side'] == 'buy' and order['info']['tradeSide'] == 'open':
        ...
        short_orders_left += 1
    okx.cancel_trigger_order(order['id'], params['symbol'])
...
data = okx.fetch_recent_ohlcv(params['symbol'], params['timeframe'], 100).iloc[:-1]
...
closed_orders = okx.fetch_closed_trigger_orders(params['symbol'])
...
if 'price_jump_pct' in params and open_position:
    if position['side'] == 'long':
        if data['close'].iloc[-1] < float(position['info']['openPriceAvg']) * (1 - params['price_jump_pct']):
            okx.flash_close_position(params['symbol'])
            update_tracker_file(tracker_file, {
                "last_side": "long",
...
if not open_position:
    okx.set_margin_mode(params['symbol'], margin_mode=params['margin_mode'])
    okx.set_leverage(params['symbol'], margin_mode=params['margin_mode'], leverage=params['leverage'])
...
if open_position:
    ...
    # exit
    okx.place_trigger_market_order(
    ...
    # sl
    sl_order = okx.place_trigger_market_order(
        symbol=params['symbol'],
        side=close_side,
        amount=amount,
...
balance = params['balance_fraction'] * params['leverage'] * okx.fetch_balance()['USDT']['total']
...
if long_ok:
    for i in range_longs:
        ...
        min_amount = okx.fetch_min_amount_tradable(params['symbol'])
        if amount >= min_amount:
            # entry           
            okx.place_trigger_limit_order(
                symbol=params['symbol'],
                side='buy',
                amount=amount,
