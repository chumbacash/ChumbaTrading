...
class OKXFutures():
    ...
    def fetch_open_positions(self, symbol: str) -> List[Dict[str, Any]]:
        try:
            positions = self.session.fetch_positions([symbol], params={'productType': 'UM', 'marginCoin': 'USDT'})
            real_positions = []
    ...
    def set_margin_mode(self, symbol: str, margin_mode: str = 'isolated') -> None:
        try:
            self.session.set_margin_mode(
                margin_mode,
                symbol,
                params={'productType': 'UM', 'marginCoin': 'USDT'},
            )
        except Exception as e:
    ...
    def fetch_recent_ohlcv(self, symbol: str, timeframe: str, limit: int = 1000) -> pd.DataFrame:
        ...
        while current_timestamp < end_timestamp:
            request_end_timestamp = min(current_timestamp + (okx_fetch_limit * timeframe_to_milliseconds[timeframe]),
                                        end_timestamp)
            try:
            ...
            current_timestamp += (okx_fetch_limit * timeframe_to_milliseconds[timeframe]) + 1
